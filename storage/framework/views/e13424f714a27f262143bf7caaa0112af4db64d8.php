...
<?php $__env->startSection('content'); ?>
            ...
            <div id=" main-content">
                <div id="wp-list" class="row">
                    <div id="wp-left-list" class="col-md-9">
                        ...
                        <ul class="wp-product row">
                            <?php if(count($list_products) > 0): ?>
                                <?php $__currentLoopData = $list_products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li class="col-6 col-sm-4 col-lg-4 prHA" style="position: relative;">
                                        <div class="card-prd">
                                            ...
                                            <div class="cprd-body">
                                                <a href="<?php echo e(route('productDetail', [$product->slug, $product->id])); ?>"
                                                    ...
                                                    <?php echo e(Str::of($product->product_title)->limit(48)); ?>
                        ...
                        <?php if(count($list_products) == 0): ?>
                            <div id="pagination_product" class="text-center" style="margin-bottom: 25px;"
                                data_uri="<?php echo e(route('pagging_products')); ?>" cat_id="<?php echo e($id); ?>">
                                <?php echo e($productChildren->links()); ?>

                            </div>
                        <?php else: ?>
                            <div id="paginationForFilterProduct" class="text-center" style="margin-bottom: 25px;"
                                data_uri="<?php echo e(route('pagging_products')); ?>" cat_id="<?php echo e($id); ?>">
                                <?php
                                    echo pagging_products($num_page, $page, route('productCat', [$slug, $id]));
                                ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div id="sort" class="col-md-3">
                        <div class="right-head-list">
                            <p class="desc">Hiển thị 5 sản phẩm</p>
                            <div class="form-filter">
                                <form action="" method="GET">
                                    <div class="form-group">
                                        <select name="sort" class="form-control" id="sort">
                                            <option value="price-asc" <?php if($sort == 'price-asc'): ?> selected <?php endif; ?>>Từ
                                                thấp đến cao</option>
                                            <option value="price-desc" <?php if($sort == 'price-desc'): ?> selected <?php endif; ?>>Từ
                                                cao đến thấp</option>
                                        </select>
                                    </div>
                                </form>
                        ...
                        <div class="right-wp-filter">
                            <div class="section-body">
                                <div id="reset_filter" style="margin-bottom: 20px">
                                    <a href="<?php echo e(Request::url()); ?>" class="btn btn-dark form-control">Reset</a>
                                </div>
                                <div class="wp-filter-price">
                                    <h1>Mức giá</h1>
                                    <form action="" id="filter-range" method="GET"
                                        data_uri="<?php echo e(route('show_prd')); ?>" data_id="<?php echo e($id); ?>">
                                        <p class="price-show mb-3">
                                            đ<span><?php echo e(number_format(!empty($price) ? $price[0] : $min_price_range, 0, '.', ',')); ?>

                                                -</span>
                                            đ<span><?php echo e(number_format(!empty($price) ? $price[1] : $max_price_range, 0, '.', ',')); ?></span>
                                        </p>
                                ...
                                <div class="wp-other-filter">
                                    <?php if(!empty($groupIdParent3)): ?>
                                        <?php echo $__env->make('layouts.client-sidebar-3',
                                            compact('groupIdParent3', 'id'), \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                                    <?php elseif(!empty($groupIdParent2)): ?>
                                        <?php echo $__env->make('layouts.client-sidebar-2',
                                            compact('groupIdParent2', 'groupCatId'), \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                                    <?php else: ?>
                                        <?php echo $__env->make('layouts.client-sidebar-1',
                                            compact('result', 'groupCatId1'), \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // if (sort != null) {
            //     $("select#sort option[value=" + sort + "]").prop("selected", "selected")
            // }
            // var op1 = getUrlParameter('op1');
            // if (op1 != null) {
            //     let op1_load = op1.split(',');
            //     if(in_array())
            //     for (var i = 0; i < op1_load.length; i++) {
            //         var val = op1_load[i].replace("+", " ")
            //         console.log(val);
            //         $("input[value= " + val + "]").prop("checked", "checked")
            //     }
            //     // console.log(op1_load);
            // }
            function b() {
                ...
                var groupOp = op1.concat(op2, op3, op4, op5, op6, op7, op8, op9, op10);
                ...
                if (0 == op5.length) e.delete("op5");
                else {
                    var w = op5.toString();
                    e.set("op5", w)
                }
                if (0 == op6.length) e.delete("op6");
                else {
                    var y = op6.toString();
                    e.set("op6", y)
                }
                if (0 == op7.length) e.delete("op7");
                else {
                    var z = op7.toString();
                    e.set("op7", z)
                }
                if (0 == op8.length) e.delete("op8");
                else {
                    var q = op8.toString();
                    e.set("op8", q)
                }
                if (0 == op9.length) e.delete("op9");
                else {
                    var p = op9.toString();
                    e.set("op9", p)
                }
                if (0 == op10.length) e.delete("op10");
                else {
                    var l = op10.toString();
                    e.set("op10", l)
                }
...
<?php echo $__env->make('layouts.client', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp-1\htdocs\Laptop88\resources\views/client/product-cat/index.blade.php ENDPATH**/ ?>