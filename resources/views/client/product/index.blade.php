...
@section('title', $productDetail->product_title)
@section('content')
    <div id="main-content-wp" class="wp-homepage">
        <div id="wp-inner" class="container">
            @include('layouts.breadcrumb', compact('productDetail', 'id', 'slugLastCat', 'productCatTitle'))
            <div class="main-content pt-4">
                <form action="{{ route('cart.addNormal', $productDetail->id) }}" method="POST">
                    @csrf
                    <div class="section" id="detail-product-wp">
                        <div class="section-detail clearfix box-left">
                            <div class="thumb-wp fl-left">
                                <a href="" title="" id="main-thumb">
                                    <img style="cursor: pointer;" src="{{ url($productDetail->thumbnail) }}"
                                        id="main-thumb" />
                                </a>
                                <div id="list-thumb" class="owl-carousel owl-loaded">
                                    @foreach ($subImages as $image)
                                        <img style="cursor: pointer;"
                                            src="{{ url('uploads/products/subImages/' . $image->path_img) }}"
                                            class="product_thumb" />
                                    @endforeach
                                </div>
                                <div class="box-banner-sis" style="margin-top: 10px">
                                    <img src="{{ asset('client/images/gif-400x100.png') }}" alt=""
                                        title="Khai trương giàn hàng samsung">
                                </div>
                            </div>
                            <div class="info fl-right">
                                <h3 class="product-name">{{ $productDetail->product_title }}</h3>
                                <div class="extra-info">
                                    <div class="pro-code" style="margin-bottom:5px">
                                        Mã sản phẩm: <strong>{{ $productDetail->code }}</strong>
                                    </div>
                                </div>
                                <div class="pro_qty">
                                    <span class="title">Số lượng: </span>
                                    <span
                                        class="qty">{{ $productDetail->qty - $productDetail->products_sold }}</span>
                                </div>
                                <div class="num-product">
                                    <span class="title">Sản phẩm: </span>
                                    @if ($productDetail->qty - $productDetail->products_sold > 0)
                                ...
                                <div id="num-order-wp">
                                    ...
                                    <input type="text" name="num_order"
                                        data_qty="{{ $productDetail->qty - $productDetail->products_sold }}" value="1"
                                        id="num-order">
                                    <a title="" id="plus"><i class="fa fa-plus"></i></a>
                                </div>
                                @if (!empty($productDetail->special_offer))
                                ...
                                <div class="box-action-button">
                                    ...
                                    <div class="form-group">
                                        <div class="group-button d-flex">
                                            ...
                                            <div class="button2">
                                                <a data_id="{{ $productDetail->id }}"
                                                    data_uri="{{ route('cart.add') }}"
                                                    uri_cart_show="{{ route('cart.show') }}" name="add-cart"
                                                    class="action-button button-blue add-cart" @if ($productDetail->qty - $productDetail->products_sold > 0)
                                                @else
                                                    disabled ='disabled'
                                                    @endif>
                        ...
                        <div class="box-right sell-policy">
                          ...
                          <div id="facebook" class="d-flex" style="padding-left: 21px">
                           <div class="fb-like" data-href="{{ $url_canonical }}" style="margin-right: 10px;margin-top: 4px;" data-width="" data-layout="button" data-action="like" data-size="small" data-share="false"></div>
                           <div class="fb-share-button" data-href="{{ $url_canonical }}" data-layout="button" data-size="small"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{ $url_canonical }}" class="fb-xfbml-parse-ignore">Chia sẻ</a></div>
                             </div>
                        </div>
                    </div>
                </form>
                    ...
                    <div id="nav_product">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane show fade active" id="tab-content-1">
                                        ...
                                        <div class="row">
                                            <div class="info-related_pro col-12 d-flex">
                                                <div class="creator">
                                                    ...
                                                    <span>{{ $productDetail->user->name }}</span>
                                                </div>
                                                <div class="day_created">
                                                    <i class="far fa-calendar"></i>
                                                    <span>{{ $productDetail->created_at }}</span>
                                                </div>
                                            </div>
                                            <div class="real_rate col-12">
                                                <span class="near_round_rating">Có
                                                    <strong>{{ $near_round_rating }}</strong> trên
                                                    5</span>
                                                <ul class="list-inline real_rating">
                                                    @for ($count = 1; $count <= 5; $count++)
                                                        ...
                                                        <li class="real_rating"
                                                            style="cursor: pointer; font-size:30px; margin-right:5px; {{ $color }}"
                                                            title="real_rating">
                                                            &#9733;</li>
                                                    @endfor
                                                </ul>
                                            ...
                                            <div class="inner-rating col-12">
                                                ...
                                                <ul class="list-inline rating" data_uri="{{ route('ratingProAjax') }}"
                                                    data_product_id="{{ $productDetail->id }}">
                                                    @for ($count = 1; $count <= 5; $count++)
                                                        <li class="rating"
                                                            id="{{ $productDetail->id }}-{{ $count }}"
                                                            data_index="{{ $count }}"
                                                            data_product_id="{{ $productDetail->id }}"
                                                            data_rating="{{ $rating }}"
                                                                ...
                                                                <ul class="repeat-purchase-con d-flex">
                                                                    @for ($count = 1; $count <= 5; $count++)
                                                                        ...
                                                                        <li class="ratingOfCust"
                                                                            style="cursor: pointer; font-size:18px; margin-right:5px; {{ $color }}"
                                                                            title="ratingOfCust">
                                                                            &#9733;</li>
                                                                    @endfor
                                                                </ul>
                                ...
                                {{-- <div class="comment" style="background: #fff; margin-top: 10px">
                                <h3>Bình luận</h3>
                                <div id="fb-root"></div>
                                <script async defer crossorigin="anonymous" src="https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v12.0"
                                                            nonce="fUQwvo0O"></script>
                ...
                @if ($sameCategory->count() > 0)
                    <div class="section" id="same-category-wp" style="padding-bottom: 30px">
                        ...
                        <div class="section-detail">
                            <ul class="list-item">
                                @foreach ($sameCategory as $itemPro)
                                    <li>
                                        <a href="{{ route('productDetail', [$itemPro->slug, $itemPro->id]) }}" title=""
                                            class="thumb d-block">
                                            <img src="{{ url($itemPro->thumbnail) }}">
                                        </a>
                                        <a href="{{ route('productDetail', [$itemPro->slug, $itemPro->id]) }}" title=""
                                            class="product-name">{{ Str::of($itemPro->product_title)->limit(50) }}</a>
                                        <div class="price">
                                            <span
                                                class="new">{{ number_format($itemPro->price, 0, ',', '.') }}đ</span>
                                        </div>
                                        <div class="action text-center">
                                            <a href="" data_id="{{ $itemPro->id }}" data_uri="{{ route('cart.add') }}"
                                                uri_cart_show="{{ route('cart.show') }}" title=""
