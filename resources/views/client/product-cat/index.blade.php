@php
use App\Parameter_pro;
use App\Parameterpro_cat;
@endphp
@extends('layouts.client')
@section('title', $productCatTitle)
@section('content')
    <div id="main-content-wp" class="wp-homepage">
        <div id="wp-inner" class="container">
            @include('layouts.breadcrumb', compact('slugLastCat', 'id'))
            <div id=" main-content">
                <div id="wp-list" class="row">
                    <div id="wp-left-list" class="col-md-9">
                        <div id="head-list">
                            <div class="cat-name">{{ $productCatTitle }}</div>
                        </div>
                        <ul class="wp-product row">
                            @if (count($list_products) > 0)
                                @foreach ($list_products as $product)
                                    <li class="col-6 col-sm-4 col-lg-4 prHA" style="position: relative;">
                                        <div class="card-prd">
                                            <a href="{{ route('productDetail', [$product->slug, $product->id]) }}"
                                                class="pro-thumb d-block">
                                                <img src="{{ url($product->thumbnail) }}" alt="" class="d-block">
                                            </a>
                                            <div class="cprd-body">
                                                <a href="{{ route('productDetail', [$product->slug, $product->id]) }}"
                                                    class="pro-name d-block">
                                                    {{ Str::of($product->product_title)->limit(48) }}
                                                </a>
                                                <span
                                                    class="pro-price text-center">{{ number_format($product->price, 0, ',', '.') }}đ</span>
                                                <div class="btn_of_prd">
                                                    <a href="" data_id="{{ $product->id }}"
                                                        data_uri="{{ route('cart.add') }}"
                                                        uri_cart_show="{{ route('cart.show') }}"
                                                        class="add-cart mt-2 mb-3 btn btn-outline-dark">Thêm giỏ
                                                        hàng</a>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
                            @else
                                @foreach ($productChildren as $product)
                                    <li class="col-6 col-sm-4 col-lg-4 prHA" style="position: relative;">
                                        <div class="card-prd">
                                            <a href="{{ route('productDetail', [$product->slug, $product->id]) }}"
                                                class="pro-thumb d-block">
                                                <img src="{{ url($product->thumbnail) }}" alt="" class="d-block">
                                            </a>
                                            <div class="cprd-body">
                                                <a href="{{ route('productDetail', [$product->slug, $product->id]) }}"
                                                    class="pro-name d-block">
                                                    {{ Str::of($product->product_title)->limit(48) }}
                                                </a>
                                                <span
                                                    class="pro-price text-center">{{ number_format($product->price, 0, ',', '.') }}đ</span>
                                                <div class="btn_of_prd">
                                                    <a href="" data_id="{{ $product->id }}"
                                                        data_uri="{{ route('cart.add') }}"
                                                        uri_cart_show="{{ route('cart.show') }}"
                                                        class="add-cart mt-2 mb-3 btn btn-outline-dark">Thêm giỏ
                                                        hàng</a>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
                            @endif
                        </ul>
                        @if (count($list_products) == 0)
                            <div id="pagination_product" class="text-center" style="margin-bottom: 25px;"
                                data_uri="{{ route('pagging_products') }}" cat_id="{{ $id }}">
                                {{ $productChildren->links() }}
                            </div>
                        @else
                            <div id="paginationForFilterProduct" class="text-center" style="margin-bottom: 25px;"
                                data_uri="{{ route('pagging_products') }}" cat_id="{{ $id }}">
                                @php
                                    echo pagging_products($num_page, $page, route('productCat', [$slug, $id]));
                                @endphp
                            </div>
                        @endif
                    </div>
                    <div id="sort" class="col-md-3">
                        <div class="right-head-list">
                            <p class="desc">Hiển thị 5 sản phẩm</p>
                            <div class="form-filter">
                                <form action="" method="GET">
                                    <div class="form-group">
                                        <select name="sort" class="form-control" id="sort">
                                            <option value="price-asc" @if ($sort == 'price-asc') selected @endif>Từ
                                                thấp đến cao</option>
                                            <option value="price-desc" @if ($sort == 'price-desc') selected @endif>Từ
                                                cao đến thấp</option>
                                        </select>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <div class="right-wp-filter">
                            <div class="section-body">
                                <div id="reset_filter" style="margin-bottom: 20px">
                                    <a href="{{ Request::url() }}" class="btn btn-dark form-control">Reset</a>
                                </div>
                                <div class="wp-filter-price">
                                    <h1>Mức giá</h1>
                                    <form action="" id="filter-range" method="GET"
                                        data_uri="{{ route('show_prd') }}" data_id="{{ $id }}">
                                        <p class="price-show mb-3">
                                            đ<span>{{ number_format(!empty($price) ? $price[0] : $min_price_range, 0, '.', ',') }}
                                                -</span>
                                            đ<span>{{ number_format(!empty($price) ? $price[1] : $max_price_range, 0, '.', ',') }}</span>
                                        </p>
                                        {{-- <form action=""> --}}
                                        <input type="hidden" id="save_min" value="{{ $min_price }}">
                                        <input type="hidden" id="save_max" value="{{ $max_price }}">
                                        <input type="hidden" name="start_price" id="start_price"
                                            value="{{ $min_price }}">
                                        <input type="hidden" name="end_price" id="end_price" value="{{ $max_price }}">
                                        <div id="slider-range">
                                        </div>
                                    </form>
                                </div>
                                <div class="wp-other-filter">
                                    @if (!empty($groupIdParent3))
                                        @include('layouts.client-sidebar-3',
                                            compact('groupIdParent3', 'id'))
                                    @elseif(!empty($groupIdParent2))
                                        @include('layouts.client-sidebar-2',
                                            compact('groupIdParent2', 'groupCatId'))
                                    @else
                                        @include('layouts.client-sidebar-1',
                                            compact('result', 'groupCatId1'))
                                    @endif
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script>
        // $("div.right-wp-filter div.section-body input").click(function() {
        $(document).ready(function() {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });

            function a(a) {
                var b = [];
                return $("." + a + ":checked").each(function() {
                    b.push($(this).val())
                }), b
            }
            // var sort = getUrlParameter('sort');
            // if (sort != null) {
            //     $("select#sort option[value=" + sort + "]").prop("selected", "selected")
            // }
            // var op1 = getUrlParameter('op1');
            // if (op1 != null) {
            //     let op1_load = op1.split(',');
            //     if(in_array())
            //     for (var i = 0; i < op1_load.length; i++) {
            //         var val = op1_load[i].replace("+", " ")
            //         console.log(val);
            //         $("input[value= " + val + "]").prop("checked", "checked")
            //     }
            //     // console.log(op1_load);
            // }
            function b() {
                const url = new URL(window.location);
                var URI = $("#filter-range").attr('data_uri'),
                    id = $("#filter-range").attr('data_id'),
                    op1 = a("op1"),
                    op2 = a("op2"),
                    op3 = a("op3"),
                    op4 = a("op4"),
                    op5 = a("op5"),
                    op6 = a("op6"),
                    op7 = a("op7"),
                    op8 = a("op8"),
                    op9 = a("op9"),
                    op10 = a("op10"),
                    save_min = $("#save_min").val(),
                    save_max = $("#save_max").val(),
                    start_price = $("input#start_price").val(),
                    end_price = $("input#end_price").val(),
                    sort = $("select[name='sort']").val(),
                    page = 1,
                    groupOp = [],
                    price_range = [],
                    e = url.searchParams;
                price_range.push(start_price);
                price_range.push(end_price);
                var groupOp = op1.concat(op2, op3, op4, op5, op6, op7, op8, op9, op10);
                if (0 == op1.length) e.delete("op1");
                else {
                    var t = op1.toString();
                    // sessionStorage.setItem("op1", t);
                    // alert(sessionStorage.getItem('op1'));
                    e.set("op1", t)
                }
                if (0 == op2.length) e.delete("op2");
                else {
                    var r = op2.toString();
                    e.set("op2", r)
                }
                if (0 == op3.length) e.delete("op3");
                else {
                    var s = op3.toString();
                    e.set("op3", s)
                }
                if (0 == op4.length) e.delete("op4");
                else {
                    var u = op4.toString();
                    e.set("op4", u)
                }
                if (0 == op5.length) e.delete("op5");
                else {
                    var w = op5.toString();
                    e.set("op5", w)
                }
                if (0 == op6.length) e.delete("op6");
                else {
                    var y = op6.toString();
                    e.set("op6", y)
                }
                if (0 == op7.length) e.delete("op7");
                else {
                    var z = op7.toString();
                    e.set("op7", z)
                }
                if (0 == op8.length) e.delete("op8");
                else {
                    var q = op8.toString();
                    e.set("op8", q)
                }
                if (0 == op9.length) e.delete("op9");
                else {
                    var p = op9.toString();
                    e.set("op9", p)
                }
                if (0 == op10.length) e.delete("op10");
                else {
                    var l = op10.toString();
                    e.set("op10", l)
                }
                start_price == save_min && end_price == save_max ? e.delete('price') : e.set('price', price_range),
                    e.set('sort', sort), e.set('page', page);
                let A = url.toString();
                history.replaceState("", "", A);
                $.ajax({
                    type: "post",
                    url: URI,
                    data: {
                        id: id,
                        start_price: start_price,
                        end_price: end_price,
                        price_range: price_range,
                        page: page,
                        sort: sort,
                        groupOp: groupOp,
                        save_min: save_min,
                        save_max: save_max,
                    },
                    dataType: "json",
                    success: function(data) {
                        $("div#wp-left-list").html(data);
                        // $("#wp_mw_row_prd").html(a.html), $("#current").text(a.curr), $("#on").text(a
                        //     .total)
                    }
                })
            };
            $("div.right-wp-filter div.section-body input").click(function() {
                b();
            });
            $(document).on("change", "select#sort", function() {
                b();
            });
            $("#slider-range").slider({
                orientation: "horizontal",
                range: true,
                min: {{ $min_price_range }},
                max: {{ $max_price_range }},
                step: 1e3,
                values: [{{ $min_price }}, {{ $max_price }}],
                stop: function(a, ui) {
                    $("p.price-show").html(new Intl.NumberFormat("en-US", {
                        style: "currency",
                        currency: "VND"
                    }).format(ui.values[0]) + " - " + new Intl.NumberFormat("en-US", {
                        style: "currency",
                        currency: "VND"
                    }).format(ui.values[1])), $("#start_price").val(ui.values[0]), $(
                        "#end_price").val(ui.values[1]), b()
                }
            });
            $(document).on('click', 'div#paginationForFilterProduct li a.page-link', function(e) {
                e.preventDefault();
                var page_id = $(this).attr('href').split('page=')[1];
                const url = new URL(window.location);
                var URI = $("#filter-range").attr('data_uri'),
                    id = $("#filter-range").attr('data_id'),
                    op1 = a("op1"),
                    op2 = a("op2"),
                    op3 = a("op3"),
                    op4 = a("op4"),
                    op5 = a("op5"),
                    op6 = a("op6"),
                    op7 = a("op7"),
                    op8 = a("op8"),
                    op9 = a("op9"),
                    op10 = a("op10"),
                    save_min = $("#save_min").val(),
                    save_max = $("#save_max").val(),
                    start_price = $("input#start_price").val(),
                    end_price = $("input#end_price").val(),
                    sort = $("select[name='sort']").val(),
                    groupOp = [],
                    price_range = [],
                    e = url.searchParams;
                price_range.push(start_price);
                price_range.push(end_price);
                var groupOp = op1.concat(op2, op3, op4, op5, op6, op7, op8, op9, op10);
                if (0 == op1.length) e.delete("op1");
                else {
                    var t = op1.toString();
                    e.set("op1", t)
                }
                if (0 == op2.length) e.delete("op2");
                else {
                    var r = op2.toString();
                    e.set("op2", r)
                }
                if (0 == op3.length) e.delete("op3");
                else {
                    var s = op3.toString();
                    e.set("op3", s)
                }
                if (0 == op4.length) e.delete("op4");
                else {
                    var u = op4.toString();
                    e.set("op4", u)
                }
                if (0 == op5.length) e.delete("op5");
                else {
                    var w = op5.toString();
                    e.set("op5", w)
                }
                if (0 == op6.length) e.delete("op6");
                else {
                    var y = op6.toString();
                    e.set("op6", y)
                }
                if (0 == op7.length) e.delete("op7");
                else {
                    var z = op7.toString();
                    e.set("op7", z)
                }
                if (0 == op8.length) e.delete("op8");
                else {
                    var q = op8.toString();
                    e.set("op8", q)
                }
                if (0 == op9.length) e.delete("op9");
                else {
                    var p = op9.toString();
                    e.set("op9", p)
                }
                if (0 == op10.length) e.delete("op10");
                else {
                    var l = op10.toString();
                    e.set("op10", l)
                }
                start_price == save_min && end_price == save_max ? e.delete('price') : e.set('price',
                        price_range),
                    e.set('sort', sort), e.set('page', page_id);
                let A = url.toString();
                history.replaceState("", "", A);
                $.ajax({
                    type: "post",
                    url: URI,
                    data: {
                        id: id,
                        start_price: start_price,
                        end_price: end_price,
                        price_range: price_range,
                        page: page_id,
                        sort: sort,
                        groupOp: groupOp,
                        save_min: save_min,
                        save_max: save_max,
                    },
                    dataType: "json",
                    success: function(data) {
                        $("div#wp-left-list").html(data);
                    }
                })
            });
            // sessionStorage.setItem("op1", a('op1'));
            // window.onload = function() {
            //     //     // If sessionStorage is storing default values (ex. name), exit the function and do not restore data
            //     //     // if (sessionStorage.getItem('') == "name") {
            //     //     //     return;
            //     //     // }
            //     //     // If values are not blank, restore them to the fields
            //     var op1 = sessionStorage.getItem('op1');
            //     console.log(op1);
            //     if (op1 != null) {
            //         document.getElementById(op1).checked = true;
            //         b();
            //     }
            //     //     // var email = sessionStorage.getItem('email');
            //     //     // if (email !== null) $('#inputEmail').val(email);
            //     //     // var subject = sessionStorage.getItem('subject');
            //     //     // if (subject !== null) $('#inputSubject').val(subject);
            //     //     // var message = sessionStorage.getItem('message');
            //     //     // if (message !== null) $('#inputMessage').val(message);
            // };
            // // Before refreshing the page, save the form data to sessionStorage
            // window.onbeforeunload = function() {
            //     sessionStorage.setItem("op1", a("op1"));
            //     // sessionStorage.setItem("email", $('#inputEmail').val());
            //     // sessionStorage.setItem("subject", $('#inputSubject').val());
            //     // sessionStorage.setItem("message", $('#inputMessage').val());
            // };
        })
    </script>
@endsection
